             ...
             <tbody>
              ...
              <tr>
               ...
               <td>
                ...
                <td>
               <?php   
                 $allready = DB::table('purchase_orders')->select('*')->where([['request_id',$vendor_quotation_lists->request_id],['category_id',3]])->count();           ?>
                  @if($allready==0)
                    <a href="{{route('purchases.show',[$vendor_quotation_lists->id,'single_vendor_purchase_order'])}}" class="btn btn-primary m-b-10 m-l-5"><i class="fa fa-check-circle"></i> Purchase Order</a>
                   @else
                    <a href="{{route('purchases.show',[$vendor_quotation_lists->id,'single_vendor_purchase_order'])}}" class="btn btn-primary m-b-10 m-l-5"><i class="fa fa-search"></i>View</a>
                   @endif
              </td>
            </tr>
            @endforeach
